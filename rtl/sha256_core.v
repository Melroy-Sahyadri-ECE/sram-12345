`timescale 1ns / 1ps
// ============================================================================
// SHA-256 Core (Cleaned Up)
//
// Standard SHA-256 hash: 512-bit input → 256-bit output
// Pipeline: IDLE → PREPARE (64 cycles) → COMPRESS (64 cycles) → FINALIZE
// ============================================================================
...
module sha256_core (
    ...
    input  wire         rst,
    input  wire         start,
    input  wire [511:0] message_block,  // 512-bit padded message
    output reg  [255:0] hash_out,       // 256-bit hash
    output reg          done
);
    ...
    function [31:0] Sigma0;   // Upper-case sigma 0
        input [31:0] x;
        Sigma0 = rotr(x,2) ^ rotr(x,13) ^ rotr(x,22);
    endfunction

    function [31:0] Sigma1;   // Upper-case sigma 1
        input [31:0] x;
        Sigma1 = rotr(x,6) ^ rotr(x,11) ^ rotr(x,25);
    endfunction

    function [31:0] sigma0;   // Lower-case sigma 0
        input [31:0] x;
        sigma0 = rotr(x,7) ^ rotr(x,18) ^ (x >> 3);
    endfunction

    function [31:0] sigma1;   // Lower-case sigma 1
        input [31:0] x;
    ...
    // Working Variables
    // ========================================================================
    reg [31:0] H0, H1, H2, H3, H4, H5, H6, H7;  // Hash state
    reg [31:0] a, b, c, d, e, f, g, h;            // Round variables
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            case (state)
                ...
                IDLE: begin
                    if (start) begin
                        // Load initial hash values (NIST constants)
                        ...
                        // Working variables = initial hash
                ...
                PREPARE: begin
                    if (round < 16) begin
                        // W[0..15] from input block (big-endian 32-bit words)
                        W[round] <= message_block[511 - round*32 -: 32];
                    end
                    else begin
                        // W[16..63] extended using sigma functions
                ...
                // Output final hash = initial + compressed
                FINALIZE: begin
